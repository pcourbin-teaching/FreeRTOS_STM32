...
#define MODE 1 // 0 = graphes, 1 = console, 2 = Console+Trace
...
#define NLMIN 6  // No ligne de depart
#define NLMAX 34 // No de ligne maxi. de l'ecran
...
void Graphe_Thread(void *p)
{
	...
	//initCons();
	if (MODE == 0) prepare_display_graphs();
	while(1) {
		if (MODE == 0) display_graphs();
		else { // MODE != 0
